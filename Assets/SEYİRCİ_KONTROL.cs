...
public class SEYİRCİ_KONTROL : MonoBehaviour
{
    public float SEYİRCİ1hiz = -100f;           //YOLLARIN HIZI EKRANA AÇIK YAPILDI
    public float SEYİRCİ2hiz = -100f;
    ...
    void Update()
    {
        if (seyirci1.transform.position.z <= -200f)                           //HARAKETLİ YOL GÖRÜNÜMÜ KAZANDIRILDI. BELLEKTEN TASARRUF EDİLDİ
        {
            seyirci1.transform.position += new Vector3(0, 0, 1040);
        }
        if (seyirci2.transform.position.z <= -200f)
        {
            seyirci2.transform.position += new Vector3(0, 0, 1040);      //YOL 1 VE YOL 2 -X YÖNÜNDE 174 E GELİNCE UZUNLUĞUN 58 KATI KADAR İLERİ IŞINLANSIN 
        }
